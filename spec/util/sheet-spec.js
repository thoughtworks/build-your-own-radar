...
describe('sheet', () => {
  ...
  it('knows to find the sheet id from published URL', () => {
  ...
  it('knows to find the sheet id from publicly shared URL having query params', () => {
    const sheet = new Sheet(
      'https://docs.google.com/spreadsheets/d/sheetId?abc=xyz',
    );

    expect(sheet.id).toEqual('sheetId');
  });

  it('knows to find the sheet id from publicly shared URL having extra path and query params', () => {
    const sheet = new Sheet(
  ...
  it('knows to find the sheet id from publicly shared URL having no extra path or query params', () => {
    const sheet = new Sheet('https://docs.google.com/spreadsheets/d/sheetId');

    expect(sheet.id).toEqual('sheetId');
  });

  it('knows to find the sheet id from publicly shared URL with trailing slash', () => {
    const sheet = new Sheet('https://docs.google.com/spreadsheets/d/sheetId/');

    expect(sheet.id).toEqual('sheetId');
  });

  it('can identify a plain sheet ID', () => {
  ...
  it('calls back with error if sheet does not exist', () => {
    const mockCallback = jest.fn();
    const xhrMock = {
      open: jest.fn(),
      send: jest.fn(),
  ...
  it('should give the sheet not found error with new message', () => {
    const errorMessage =
      'Oops! We can’t find the Google Sheet you’ve entered, please check the URL of your sheet.';
    const mockCallback = jest.fn();
    const xhrMock = {
      open: jest.fn(),
      send: jest.fn(),
