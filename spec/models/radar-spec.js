...
describe('Radar', () => {
  ...
  it('has no quadrants by default', () => {
    var radar = new Radar();
  ...
  it('sets the current sheet', () => {
    const radar = new Radar();
    const sheetName = 'The current sheet';
    radar.setCurrentSheet(sheetName);
    expect(radar.getCurrentSheet()).toEqual(sheetName);
  });

  it('throws an error if too many quadrants are added', () => {
    var quadrant, radar, blip;
  ...
  describe('blip numbers', () => {
    ...
    it('sets blip numbers starting on the first quadrant', () => {
    ...
    it('continues the number from the previous quadrant set', () => {
      radar.addQuadrant(firstQuadrant);
  ...
  describe('alternatives', () => {
    it('returns a provided alternatives', () => {
      var radar = new Radar();
  ...
  describe('rings : new UI', () => {
    ...
    beforeEach(() => {
      ...
      graphConfig.rings.forEach((ring, index) =>
        rings.push(new Ring(ring, index)),
      );
      radar.addRings(rings);
    });

    it('returns an array of rings in configured order and ignore the order in which blips are provided', () => {
      quadrant.add([
      ...
      expect(radar.rings()[0].name().toLowerCase()).toEqual(
        secondRing.name().toLowerCase(),
      );
      expect(radar.rings()[1].name().toLowerCase()).toEqual(
        thirdRing.name().toLowerCase(),
      );
      expect(radar.rings()[2].name().toLowerCase()).toEqual(
        fourthRing.name().toLowerCase(),
      );
      expect(radar.rings()[3].name().toLowerCase()).toEqual(
        firstRing.name().toLowerCase(),
      );
    });

    it('should not return invalid rings other than the configured ones', () => {
