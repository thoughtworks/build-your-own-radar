...
describe('Build radar with CSV', () => {
  beforeEach(() => {
    if (
      ...
      cy.visit(
        Cypress.env('host') +
          `/?documentId=${encodeURIComponent(testConfig.CSV_FILE_URL)}`,
      );
    } else {
  ...
  context('Desktop resolution (1440px)', () => {
    ...
    it('verify group-blip tooltip text with click-to-view-all text in Radar graph', () => {
      radarPage.validateBlipToolTip(
        'blip-link-first-assess-group-new-blips',
        'Click to view all',
      );
    });
    ...
    it('verify group-blip text does not appear in quadrant view', () => {
      ...
      radarPage.validateNoBlipToolTip(
        'blip-link-first-adopt-group-existing-blips',
      );
    });
...
describe('Build radar with JSON', () => {
  beforeEach(() => {
    if (
      ...
      cy.visit(
        Cypress.env('host') +
          `/?documentId=${encodeURIComponent(testConfig.JSON_FILE_URL)}`,
      );
    } else {
      cy.visit(Cypress.env('host'));
    }
...
describe('Validate public Google Sheet', () => {
      ...
      cy.visit(
        Cypress.env('host') +
          `/?documentId=${encodeURIComponent(
            testConfig.PUBLIC_GOOGLE_SHEET_URL,
          )}`,
      );
    }
  });
  ...
  context('Desktop resolution (1440px)', () => {
    it('Validate public Google Sheet in desktop', () => {
      ...
      radarPage.validateBlipToolTip(
        'blip-link-second-trial-group-new-blips',
        'Click to view all',
      );
      radarPage.validateBlipToolTip(
