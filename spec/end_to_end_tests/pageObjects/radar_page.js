...
class RadarPage {
  constructor() {
    ...
    this.blipDescription =
      '.blip-list__item-container.expand .blip-list__item-container__description';
    ...
    this.quadrantTableRings =
      '.quadrant-table.selected .quadrant-table__ring-name';
    ...
    this.tooltip = '.d3-tip';
    this.quadrantTableBlip = (blipId) =>
      `.quadrant-table.selected .blip-list__item-container[data-blip-id="${blipId}"]`;
    this.quadrantTableBlipDescription = (blipId) =>
      `.quadrant-table.selected .blip-list__item-container[data-blip-id="${blipId}"] #blip-description-${blipId}`;
    this.radarGraphBlip = (blipId) => `a.blip-link[data-blip-id="${blipId}"]`;
    this.subnavQuadrant = (quadrantName) =>
      `.quadrant-subnav__list-item#subnav-item-${quadrantName}`;
    this.quadrantGraph = (quadrantOrder) => `.quadrant-group-${quadrantOrder}`;
    this.quadrantGraphTablet = (quadrantOrder) =>
      `#${quadrantOrder}-quadrant-mobile`;
    this.searchResultByIndex = (index) => `.ui-menu-item:nth-child(${index})`;
    this.alternateRadarsItemByIndex = (index) =>
      `.alternative-radars__list-item:nth-child(${index})`;

    this.quadrantNameGroup = (order) =>
      `.quadrant-group.quadrant-group-${order} .quadrant-name-group`;

    this.quadrantRingName = (order, index) =>
      `.quadrant-group-${order} .line-text:nth-of-type(${index})`;
  }
  ...
  validateNoBlipToolTip(blipId) {
    ...
    cy.get(this.tooltip)
      .should('have.attr', 'style')
      .and('contains', 'pointer-events: none');
  }
  ...
  validateQuadrantNames() {
    ...
    for (const quadrant of config.QUADRANT_NAMES) {
      cy.get(`${this.quadrantList}:nth-child(${i})`).should(
        'have.text',
        quadrant,
      );
      i++;
  ...
  validateAlternateRadarsForPublicGoogleSheet() {
    ...
    for (const name of config.PUBLIC_GOOGLE_SHEET_RADAR_SHEET_NAMES) {
      cy.get(`${this.alternateRadarsItems}:nth-child(${i})`).should(
        'have.text',
        name,
      );
      i++;
  ...
  validBlipHighlightedInQuadrantTable(blipId) {
    cy.get(this.quadrantTableBlip(blipId))
      .parent('.blip-list__item')
  ...
  validateActiveQuadrantInSubnav(quadrantName) {
    cy.get(this.subnavQuadrant(quadrantName)).should(
      'have.class',
      'active-item',
    );
  }
  ...
  validateActiveAlternateRadar(index) {
    cy.get(this.alternateRadarsItemByIndex(index)).should(
      'have.class',
      'active',
    );
  }
  ...
  validateQuadrantOrder() {
    ...
    for (const order of config.QUADRANT_ORDERS) {
      cy.get(this.quadrantNameGroup(order)).should(
        'have.text',
        config.QUADRANT_NAMES[i],
      );
      i += 1;
    }
  }
  ...
  validateRingOrder() {
    let i = 1;
    for (const order of config.QUADRANT_ORDERS) {
      cy.get(this.quadrantRingName(order, i)).should(
        'have.text',
        config.RING_NAMES[i - 1],
      );
      i += 1;
    }
  }
  ...
  validateActiveQuadrant(quadrantName, quadrantOrder) {
    ...
    for (const order of config.QUADRANT_ORDERS.filter(
      (order) => order !== quadrantOrder,
    )) {
