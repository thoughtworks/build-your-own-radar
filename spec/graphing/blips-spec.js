...
describe('Blips', () => {
  it('should return coordinates which fall under the first quadrant and rings provided', () => {
    const startAngle = 0;
    const minRadius = 160;
    const maxRadius = 300;
    const coordinates = calculateRadarBlipCoordinates(
      minRadius,
      maxRadius,
      startAngle,
      'first',
    ...
    const xCoordMaxValue =
      ...
      maxRadiusAfterThreshold * -1 * 0.9978403633398593 +
      graphConfig.blipWidth;
    const yCoordMaxValue =
      graphConfig.effectiveQuadrantHeight +
      maxRadiusAfterThreshold * -1 * 0.06568568557743505;
    ...
    expect(Number.parseFloat(coordinates[0].toFixed(3))).toBeLessThanOrEqual(
      Number.parseFloat(xCoordMinValue.toFixed(3)),
    );
    expect(Number.parseFloat(coordinates[1].toFixed(1))).toBeGreaterThanOrEqual(
      Number.parseFloat(yCoordMinValue.toFixed(1)),
    );
    expect(Number.parseFloat(coordinates[0].toFixed(3))).toBeLessThanOrEqual(
      Number.parseFloat(xCoordMaxValue.toFixed(3)),
    );
    expect(Number.parseFloat(coordinates[1].toFixed(3))).toBeLessThanOrEqual(
      Number.parseFloat(yCoordMaxValue.toFixed(3)),
    );
  });

  it('should return coordinates for the second quadrant and consider the border offset provided', () => {
    const startAngle = -90;
    const minRadius = 160;
    const maxRadius = 300;
    const blipWidth = 22;
  ...
  it('should return first quadrant group blip coordinates for ring1', () => {
  ...
  it('should transpose base coords for a new blip in ring1 to other three quadrants', () => {
    const newBlipBaseCoords = groupBlipsBaseCoords(0).new;

    const coordsMap = transposeQuadrantCoords(
      newBlipBaseCoords,
  ...
  it('should return first quadrant group blip coordinates for ring2 with index 1', () => {
    ...
    expect(baseCoords['existing']).toEqual([
      247.0075702088335, 369.86317046071997,
    ]);
  });

  it('should return first quadrant group blip coordinates for ring3 with index 2', () => {
  ...
  it('should return first quadrant group blip coordinates for ring4 with index 3', () => {
    const baseCoords = groupBlipsBaseCoords(3);
    expect(baseCoords.new).toEqual([408.91602532749283, 23.149928577467563]);
    expect(baseCoords['existing']).toEqual([
      368.91602532749283, 52.14992857746756,
    ]);
  });

  it('should return group blip tool tip text as "Click to view all" count is more than 15', () => {
  ...
  it('should return group blip tool tip text as all blip names if count is <= 15', () => {
    const ringBlips = mockRingBlips(15);
    const actualToolTip = getGroupBlipTooltipText(ringBlips);
  ...
  it('should return ring radius based on the ring index', () => {
  ...
  it('should return group blip assistive text for group blip', () => {
    const blip = {
      isGroup: () => true,
      ring: () => {
        return {
  ...
  it('should return group blip with appropriate values', () => {
    const ringBlips = mockRingBlips(20);
  ...
  it('should return true when the given coords are colliding with existing coords', () => {
