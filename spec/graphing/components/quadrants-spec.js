...
} = require('../../../src/graphing/components/quadrants');
jest.mock('d3', () => {
  return {
    select: jest.fn(),
  };
...
describe('Quadrants', () => {
  ...
  it('should render the text in one line if length is not above the max length', () => {
    element.innerHTML = 'Tools';
    wrapQuadrantNameInMultiLine(mockedD3Element, false, quadrantGroup, tip);
    const expectedTSpanTags = element.querySelectorAll('tspan');
  ...
  it('should render the text in two lines if length is above the max length', () => {
  ...
  it('should split the first word by hyphen and render the text in two lines if its longer than max length', () => {
  ...
  it('should split the first word by hyphen and render the text in two lines with ellipsis if its longer than max length after splitting also', () => {
  ...
  it('should render the text in two lines with ellipsis if its longer than max length', () => {
