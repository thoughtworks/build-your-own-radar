...
const Radar = function (size, radar) {
    var svg, radarElement, quadrantButtons, buttonsGroup, alternativeDiv;
    ...
    function mouseoverQuadrant(order) {
        ...
        d3.selectAll('.quadrant-group:not(.quadrant-group-' + order + ')').style('opacity', 0.6);
    }
    ...
    function selectQuadrant(order, startAngle) {
        ...
        var translateY =
            -1 * (1 - adjustY) * (size / 2 - 7) * (scale - 1) - ((1 - adjustY) / 2) * (1 - scale / 2) * size;
        ...
        d3.selectAll('.quadrant-group-' + order + ' .blip-link text').each(function () {
            ...
            d3.select(this.parentNode)
                ...
                .attr(
                    'transform',
                    'scale(' + blipScale + ')translate(' + blipTranslate * x + ',' + blipTranslate * y + ')',
                );
        });
