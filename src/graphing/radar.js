...
const { renderQuadrantTables, renderRadarQuadrants, renderMobileView } = require('./components/quadrants')
...
const Radar = function (size, radar) {
  ...
  var ringCalculator = new RingCalculator(radar.rings().length, center(size, QUADRANT_GAP))
  ...
  function plotLines(quadrantGroup, quadrant) {
    ...
    quadrantGroup
      .append('line')
      .attr('x1', center(size, QUADRANT_GAP))
      .attr('x2', center(size, QUADRANT_GAP))
      .attr('y1', startY - 2)
      .attr('y2', endY + 2)
    ...
    quadrantGroup
      .append('line')
      .attr('x1', endX)
      .attr('y1', center(size, QUADRANT_GAP))
      .attr('x2', startX)
      .attr('y2', center(size, QUADRANT_GAP))
      .attr('stroke-width', 10)
  }
  ...
  function plotQuadrant(rings, quadrant) {
    ...
    rings.forEach(function (ring, i) {
      ...
      quadrantGroup
        ...
        .attr('transform', 'translate(' + center(size, QUADRANT_GAP) + ', ' + center(size, QUADRANT_GAP) + ')')
    })
    ...
    rings.forEach(function (ring, i) {
      if (quadrant.order === 'first' || quadrant.order === 'fourth') {
        quadrantGroup
          ...
          .attr('y', center(size, QUADRANT_GAP) + 4)
          .attr('x', center(size, QUADRANT_GAP) + (ringCalculator.getRadius(i) + ringCalculator.getRadius(i + 1)) / 2)
    ...
    rings.forEach(function (ring, i) {
      if (quadrant.order === 'third' || quadrant.order === 'fourth') {
        quadrantGroup
          .append('text')
          .attr('class', 'line-text')
          .attr('y', center(size, QUADRANT_GAP) + 22)
          .attr(
            'x',
            center(size, QUADRANT_GAP) +
        ...
        quadrantGroup
          ...
          .attr('class', 'line-text')
          .attr('y', center(size, QUADRANT_GAP) + 22)
          .attr('x', center(size, QUADRANT_GAP) - (ringCalculator.getRingRadius(i) + ringCalculator.getRingRadius(i + 1)) / 2)
          .attr('text-anchor', 'middle')
          .text(ring.name())
      }
    })
  ...
  function calculateBlipCoordinates(blip, chance, minRadius, maxRadius, startAngle) {
    ...
    var x = center(size, QUADRANT_GAP) + radius * Math.cos(angle) * adjustX
    var y = center(size, QUADRANT_GAP) + radius * Math.sin(angle) * adjustY
  ...
  function selectQuadrant(order, startAngle) {
    ...
    if (featureToggles.UIRefresh2022) {
      d3.select('#radar-plot').attr('width', center(size, QUADRANT_GAP)).attr('height', center(size, QUADRANT_GAP))
  ...
  self.plot = function () {
    ...
    if (featureToggles.UIRefresh2022) {
      radarElement.style('height', size + 'px')
      svg
        .attr('id', 'radar-plot')
        .attr('width', size)
        .attr('height', size)
    ...
    _.each(quadrants, function (quadrant) {
      let quadrantGroup
      if (featureToggles.UIRefresh2022) {
        quadrantGroup = renderRadarQuadrants(size, svg, quadrant, quadrantsContainer, mouseoverQuadrant, mouseoutQuadrant)
