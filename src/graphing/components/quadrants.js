...
const { quadrantHeight, quadrantWidth, quadrantsGap, effectiveQuadrantWidth } =
  graphConfig;
...
function selectRadarQuadrant(order, startAngle, name) {
  ...
  if (window.innerWidth >= uiConfig.tabletViewWidth) {
    ...
    radarLegendsContainer.style(
      'left',
      `${window.innerWidth / 2 - getElementWidth(radarLegendsContainer) / 2}px`,
    );
  }
}
...
function renderMobileView(quadrant) {
  ...
  quadrantBtn
    ...
    .style(
      'background-image',
      `url('/images/${quadrant.order}-quadrant-btn-bg.svg')`,
    )
  ...
  if (window.scrollY >= offset) {
    ...
    if (
      ...
      radarLegendsContainer.style(
        'left',
        `${
          Number.parseFloat(prevLeft.slice(0, -2)) +
    ...
    radarLegendsContainer.style(
      'top',
      `${
        Number.parseFloat(prevTop.slice(0, -2)) +
        getScaledQuadrantHeightWithGap(scale)
      }px`,
    );
    radarLegendsContainer.style(
