...
const Sheet = (sheetReference) => {
  ...
  self.getSheet = async () => {
    try {
      self.sheetResponse = await gapi.client.sheets.spreadsheets.get({
  ...
  self.processSheetResponse = async (sheetName, createBlips, handleError) =>
    self.sheetResponse.status !== 200
  ...
  function processSheetData(sheetName, createBlips, handleError) {
    const sheetNames = self.sheetResponse.result.sheets.map(
    ...
    self
      ...
      .then((r) =>
        createBlips(
          self.sheetResponse.result.properties.title,
          r.result.values,
          sheetNames,
        ),
      )
