...
const plotRadar = (title, blips, currentRadarName, alternativeRadars) => {
...
function validateInputQuadrantOrRingName(allQuadrantsOrRings, quadrantOrRing) {
  ...
  return quadrantOrRingNames.find(
    (quadrantOrRing) => quadrantOrRing.toLowerCase() === formattedInputQuadrant,
  );
}

const plotRadarGraph = (title, blips, currentRadarName, alternativeRadars) => {
...
const GoogleSheet = (sheetReference, sheetName) => {
  var self = {};

  self.build = () => {
    var sheet = new Sheet(sheetReference);
    sheet.validate((error, apiKeyEnabled) => {
  ...
  function createBlipsForProtectedSheet(documentTitle, values, sheetNames) {
    ...
    const title = featureToggles.UIRefresh2022
      ? documentTitle
  ...
  self.authenticate = (force = false, apiKeyEnabled, callback) => {
    GoogleAuth.loadGoogle(force, async () => {
      self.error = false;
      const sheet = new Sheet(sheetReference);
      await sheet.getSheet();
      if (
        sheet.sheetResponse.status === 403 &&
        !GoogleAuth.gsiInitiated &&
        !force
      ) {
        // private sheet
        GoogleAuth.loadGSI();
      } else {
        await sheet.processSheetResponse(
          sheetName,
    ...
    d3.csv(url)
      ...
      .catch((exception) => {
        const fileNotFoundError = new FileNotFoundError(
          `Oops! We can't find the CSV file you've entered`,
        );
        plotErrorMessage(
          featureToggles.UIRefresh2022 ? fileNotFoundError : exception,
          'csv',
        );
      });
  };

  var createBlips = (data) => {
    try {
      ...
      const invalidContentError = new InvalidContentError(
        ExceptionMessages.INVALID_CSV_CONTENT,
      );
      plotErrorMessage(
        featureToggles.UIRefresh2022 ? invalidContentError : exception,
        'csv',
      );
    }
  };
...
const JSONFile = (url) => {
  ...
  self.build = () => {
    d3.json(url)
      ...
      .catch((exception) => {
        const fileNotFoundError = new FileNotFoundError(
          `Oops! We can't find the JSON file you've entered`,
        );
        plotErrorMessage(
          featureToggles.UIRefresh2022 ? fileNotFoundError : exception,
          'json',
  ...
  var createBlips = (data) => {
    try {
      ...
      const invalidContentError = new InvalidContentError(
        ExceptionMessages.INVALID_JSON_CONTENT,
      );
      plotErrorMessage(
        featureToggles.UIRefresh2022 ? invalidContentError : exception,
        'json',
...
function plotLoading(content) {
  if (!featureToggles.UIRefresh2022) {
    ...
    document.querySelector('.helper-description .loader-text').style.display =
      'block';
  }
}
...
function plotLogo(content) {
  content
    ...
    .html(
      '<a href="https://www.thoughtworks.com"><img src="/images/tw-logo.png" alt="logo"/ ></a>',
    );
}
...
function plotError(exception, fileType) {
  let message;
  let faqMessage =
    'Please check <a href="https://www.thoughtworks.com/radar/byor">FAQs</a> for possible solutions.';
  if (featureToggles.UIRefresh2022) {
...
function plotUnauthorizedErrorMessage() {
  ...
  errorContainer
    ...
    .html(message);
  const newUi = featureToggles.UIRefresh2022
    ? 'switch-account-button-newui'
    : 'switch-account-button';
  const button = errorContainer
    .append('button')
    .attr('class', `button ${newUi}`)
    .text('Switch account');
